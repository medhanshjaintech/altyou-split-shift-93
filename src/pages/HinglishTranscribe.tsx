...
Thank you for watching this tutorial on ${title.toLowerCase()}`;
  ...
  const convertLineToHinglish = (line: string): string => {
    ...
    return line.replace(/welcome to/gi, 'welcome to').replace(/video/gi, 'video').replace(/explore/gi, 'explore').replace(/concepts/gi, 'concepts').replace(/applications/gi, 'applications').replace(/understanding/gi, 'samajhna').replace(/fundamentals/gi, 'fundamentals').replace(/topic/gi, 'topic').replace(/important/gi, 'important').replace(/remember/gi, 'yaad rakhna').replace(/practice/gi, 'practice').replace(/regularly/gi, 'regularly').replace(/demonstrate/gi, 'dikhana').replace(/techniques/gi, 'techniques').replace(/strategies/gi, 'strategies').replace(/effective/gi, 'effective').replace(/experts/gi, 'experts').replace(/field/gi, 'field').replace(/apply/gi, 'apply karna').replace(/principles/gi, 'principles').replace(/daily/gi, 'daily').replace(/workflow/gi, 'workflow').replace(/thank you/gi, 'thank you').replace(/watching/gi, 'dekhne ke liye').replace(/tutorial/gi, 'tutorial');
  };
  ...
  const downloadSRT = () => {
    ...
    const file = new Blob([hinglishText], {
      type: 'text/plain'
    });
      ...
      <main className={`flex-1 transition-all duration-300 ${isSidebarOpen ? 'ml-64' : 'ml-16'}`}>
        <ScrollArea className="h-screen">
          <div className="container mx-auto px-6 py-8">
            <div className="mb-8">
              <Button variant="ghost" className="mb-4 text-white hover:bg-neutral-800" onClick={() => navigate('/dashboard')}>
            ...
            {!transcriptionData ? <div className="max-w-lg mx-auto mb-12">
                <div className="bg-neutral-800 border border-neutral-700 rounded-md p-6">
                  ...
                  <div className="mb-4">
                    ...
                    <Input type="text" placeholder="https://www.youtube.com/watch?v=" value={mediaLink} onChange={handleInputChange} className="bg-blue-50 border border-blue-100 text-neutral-800" />
                  </div>
                  
                  <Button className="w-full bg-blue-500 hover:bg-blue-600 text-white flex items-center justify-center gap-2" disabled={isLoading} onClick={handleImportFromLink}>
                    {isLoading ? "Importing..." : <>
                        <span>+</span>
                        <span>IMPORT</span>
                      </>}
                  ...
                  <div className="mt-6">
                    ...
                    <div onClick={handleFileButtonClick} onDrop={e => {
                  e.preventDefault();
                ...
                }} onDragOver={e => e.preventDefault()} className="border-2 border-dashed border-neutral-600 rounded-md p-6 text-center cursor-pointer transition bg-zinc-800">
                  ...
                  <CardContent className="p-0">
                    <div className="border-b border-neutral-700 p-4">
                      <div className="flex items-center justify-between">
                        ...
                        <Button variant="ghost" size="sm" className="text-neutral-400 hover:text-white" onClick={handleCloseTranscription}>
                          <X className="h-4 w-4" />
                        </Button>
                      </div>
                      {transcriptionData.mediaType === 'video' ? <div className="flex items-center space-x-2">
                    ...
                    {isTranscribing ? <div className="flex items-center justify-center h-64 bg-neutral-900 rounded-md">
                        ...
                        <div className="border-r border-neutral-700">
                          <div className="p-6">
                            ...
                            <div className="aspect-video bg-neutral-700 rounded-md mb-6">
                              {transcriptionData.mediaType === 'video' ? <video ref={videoRef} controls className="w-full h-full rounded-md" poster="https://placehold.co/640x360/232323/606060?text=Video+Player">
                                  <source src={transcriptionData.mediaSource} type="video/mp4" />
                                  Your browser does not support the video tag.
                                </video> : <div className="w-full h-full flex flex-col items-center justify-center bg-neutral-700 p-4 rounded-md">
